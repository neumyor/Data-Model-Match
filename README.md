...
- 使用 `pyarrow` 读取 Parquet schema 与行数元数据，支持常见图像、分类标签、检测标注和分割掩码结构，以及本地图像序列的 CVAT 跟踪 XML 标注；
- 以卡片、详情和执行轨迹管理本地资源快照；
- 通过真实 LLM API 补充语义字段映射，并生成结构化兼容性报告；
- 对“转换后可用”的结果执行受约束的智能体适配闭环：计划校验、文件转换、产物重新解析和再次兼容性分析；只有复检为“兼容”才登记派生数据集；
...
1. 在“数据集管理”中输入 Hugging Face 仓库或搜索关键词；导入本地数据时选择“本地文件夹”，从文件系统选取目录。选择来源类型和下载策略后开始解析。
2. 在“模型管理”中输入 GitHub 仓库或搜索关键词；导入本地模型时选择“本地文件夹”，从文件系统选取目录。系统固定远程 commit 并静态提取模型契约。
3. 在资源详情中检查结构化描述、文件统计、解析证据、警告和完整执行轨迹。
4. 在“匹配工作台”选择一个已就绪数据集和模型，查看流式执行轨迹和兼容性报告。
5. 当报告为“转换后可用”时，可创建适配数据集。系统会限制智能体只能选择已有字段和模型声明的预处理，随后重新解析产物并再次分析；未达到“兼容”不会登记为成功。
